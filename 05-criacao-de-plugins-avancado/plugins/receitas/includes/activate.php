...
// Versão do wordpress, versão do worpress que queremos que o plugin rode (no caso, à partir da 4.5), '<' é a comparação que queremos fazer, no caso se 4.5 < 4.8, então não passará no verificação
	...
	// Criando as opções extras do nosso plugin para serem adicionadas na tabela wp_options do banco de dados

	$receitas_opts = get_option('sr_receita_opts');
	...
	if(!$receitas_opts){
		...
		add_option('sr_receita_opts', $opts);
